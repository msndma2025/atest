...
The script will:
1. Convert each `*_buildings.shp` to WGS84 GeoJSON (handles missing .shx automatically)
2. Add `height`, `base_height`, and `district` properties to every feature
3. Merge all districts into one GeoJSON
4. Run tippecanoe to produce `buildings.mbtiles`
5. Delete the intermediate GeoJSON files

Output at: `C:\HPC\tiles\buildings.mbtiles`
...
```
http://localhost:3000/data/buildings.json
```

You should see JSON with `"tiles": ["http://...3000/data/buildings/{z}/{x}/{y}.pbf"]`.
...
The file currently maps district names to WFS URLs from the CSV.
Replace it entirely — it now maps district names to the shared tile URL:

```js
// buildingData.js
// District building data now served as vector tiles from tileserver-gl.
// One combined tile source covers all districts; MapContainer filters by
// the 'district' property that convert.sh baked into every feature.

const TILE_SERVER = import.meta.env.VITE_TILE_SERVER_URL
  || 'http://172.18.1.174:3000';

export const BUILDINGS_TILE_URL =
  `${TILE_SERVER}/data/buildings/{z}/{x}/{y}.pbf`;

// Still reads the CSV so district names stay available for other uses,
// but wfsUrl is no longer used for building visualisation.
...
export async function loadBuildingIndex() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!shp) continue;
    const districtKey = shp.replace(/_buildings\.shp$/i, '');
...
Also add to `vite.config.js` (so the env var reaches the client bundle):
```js
// inside defineConfig → define:
...
**What to remove:**
- The `useEffect` around line 394–455 that fetches the WFS URL, adds a `geojson` source, and adds `district-buildings-fill` / `district-buildings-line` / `district-buildings-3d` layers.
- The `buildingFetchRef` and `activeBuildingRef` refs.
- The import of `loadBuildingIndex` / `findBuildingWfs` (if present).

**What to add:**

1. Import the new tile URL at the top of the file:
```js
import { BUILDINGS_TILE_URL } from '../../utils/buildingData';
```

2. Inside `map.on('load', () => { ... })` (alongside `loadDistrictsLayer`, `loadFloodLayers` etc.), add the vector tile source and 3D layer once:
```js
// ── Vector tile buildings source (all districts, loaded once) ─────────────
map.addSource('district-buildings-src', {
  type: 'vector',
  tiles: [BUILDINGS_TILE_URL],
  minzoom: 4,
  maxzoom: 16,
});

// 3D extrusion — initially nothing shown (empty district filter)
map.addLayer({
  id: 'district-buildings-3d',
  type: 'fill-extrusion',
  source: 'district-buildings-src',
  'source-layer': 'buildings',          // must match LAYER_NAME in convert.sh
  filter: ['==', ['get', 'district'], ''],   // show nothing until district selected
  paint: {
...
// Flat outline for context at lower zooms
map.addLayer({
...
3. Replace the WFS `useEffect` with a simpler filter-update effect:
```js
// When the selected district changes, filter the pre-loaded tile layer
useEffect(() => {
  const map = mapRef.current;
  if (!map || !mapLoaded) return;

  const districtKey = activeBuildingDistrict?.districtKey || '';
  const filterExpr = districtKey
    ? ['==', ['get', 'district'], districtKey]
    : ['==', ['get', 'district'], ''];

  ['district-buildings-3d', 'district-buildings-line'].forEach((id) => {
    if (map.getLayer(id)) map.setFilter(id, filterExpr);
  });

  // Zoom to district bounds when a district is selected
  if (districtKey && districtsGeoJSON) {
    const feat = districtsGeoJSON.features.find(
      (f) => f.properties.name === districtKey || f.properties.name?.toLowerCase() === districtKey.toLowerCase()
    );
    if (feat) {
...
}, [activeBuildingDistrict, mapLoaded, districtsGeoJSON]);
```

4. In `Sidebar.jsx`, where `activeBuildingDistrict` is assembled, change from passing
`{ wfsUrl, districtKey, qualifiedName }` to just `{ districtKey }`:
```js
// Before
